...
class ReversionStrategy:
    def __init__(self):
        # 🔹 [초기화] 각 종목별 잔여 수량 및 실행 기록을 저장하는 저장소입니다.
        self.residual = {
            "BUY1": {}, "BUY2": {}, 
        ...
        # 🔹 [U-Curve 가중치] 5년 백테스트로 검증된 시간대별 거래량 가중치 리스트입니다.
    ...
    def _save_state(self, ticker):
        # 🔹 [원자적 저장] 현재 메모리의 상태를 파일로 안전하게(Atomic Write) 저장합니다.
        today_str = datetime.now().strftime("%Y-%m-%d")
        state_file = self._get_state_file(ticker)
        data = {
            "date": today_str,
        ...
        try:
            ...
            # MODIFIED: [파일 디스크립터 누수 방어] 안전하게 파일을 쓰고 동기화합니다.
    ...
    def save_daily_snapshot(self, ticker, plan_data):
        # 🔹 [스냅샷 박제] 장 오픈 시 결정된 타점을 하루 종일 보존하기 위해 저장합니다.
    ...
    def load_daily_snapshot(self, ticker):
        # 🔹 [스냅샷 로드] 오늘 이미 생성된 지시서가 있다면 이를 불러옵니다.
        snap_file = self._get_snapshot_file(ticker)
        if os.path.exists(snap_file):
            try:
    ...
    def reset_residual(self, ticker):
        # 🔹 [초기화] 특정 종목의 잔여 계산 값을 모두 0으로 비웁니다.
    ...
    def record_execution(self, ticker, side, qty, exec_price):
        # 🔹 [체결 기록] 실제 주문이 체결된 수량과 금액을 장부에 업데이트합니다.
        self._load_state_if_needed(ticker)
    ...
    def get_dynamic_plan(self, ticker, curr_p, prev_c, current_weight, vwap_status, min_idx, alloc_cash, q_data, is_snapshot_mode=False):
        # 🔹 [핵심 엔진] 현재 시장 상황과 내 장부를 대조하여 살지 팔지(Plan)를 결정합니다.
        ...
        # MODIFIED: [결측치 방어] 시간 인덱스가 비정상일 경우 안전하게 무시합니다.
        ...
        # MODIFIED: [정상 로트 선별] 가격이 0원인 비정상 데이터는 계산에서 제외합니다.
        ...
        # 🔹 [LIFO 지층 분석] 가장 최근에 산 물량(1층)과 그 위의 물량(Upper)을 분리합니다.
        ...
        # 🔹 [공수 전환 판별] 0주라면 공격적 매수, 물량이 있다면 방어적 매매를 수행합니다.
        if total_q == 0:
            side = "BUY"
        ...
        # 🔹 [주문 생성] 강세장/약세장 시그널이 있거나 스냅샷 생성 시 주문 목록을 만듭니다.
        ...
        # 🔹 [실시간 VWAP 슬라이싱] 시간 흐름에 따라 주문 수량을 정교하게 분배합니다.
